...
//confirms a proper team name was passed in from the command line:  If it wasn't the program exits.


if (empty($TeamName) ==  true)
{
   echo __FILE__.":".__LINE__.": No records were returned! You didn't provide a current NBA team nickname. Please note input is case sensitive and examples include Knicks, Rockets, and Pacers. Please note Partial names are accepted if the case sensitivity is correct!.".PHP_EOL;
   exit(0);
}

//Prepare variable to be passed into like clause for partial name parameters

$TeamName_concat = "%" . $TeamName . "%";
$TeamName_concat_final  = $TeamName_concat;
...
$TeamName_concat_final_esc = $db->escape_string($TeamName_concat_final);
...
$query = "select Name, Abbr, ArenaName, City from team where season = 2015 and name like '$TeamName_concat_final_esc';";
...
echo $TeamDetails['Name'] . ' ';
echo $TeamDetails['Abbr'] . ' ';
echo $TeamDetails['ArenaName'] . ' ';
echo $TeamDetails['City'] . "\n";
